...
static int ac108_update_bits(u8 reg, u8 mask, u8 val, struct i2c_client *client) {
	...
	u8 val_test;
       ...
       //dev_dbg(dai->dev, "xx4xx ac108_update_bits\n");
	ac108_read(reg, &val_old, client);
	//printk("xx4.1xx ac108_update_bits  read  val_old: %u\n",val_old);
	//ac108_write(reg, val, client);
	//printk("xx4.2xx ac108_update_bits  write val_new: %u\n",val);
	//ac108_read(reg, &val_test, client);
	val_new = (val_old & ~mask) | (val & mask);
	printk("xx4.3xx ac108_update_bits   :0x%02x:0x%02x:0x%02x:0x%02x\n",reg,val_old,val_new,mask);
	if (val_new != val_old) {
	//	printk("xx4.1xx ac108_update_bits  val_new:%u\n",val_new);
		ac108_write(reg, val_new, client);
		ac108_read(reg, &val_test, client);
		printk("xx4.2xx ac108_update_bits  :0x%02x:0x%02x:0x%02x:0x%02x\n",reg,val_new,val_test,mask);
	}
	//printk("xx4.3xx ac108_update_bits :0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n",reg,val_old,val_new,val_test,mask);
...
static int ac108_multi_chips_write(u8 reg, u8 val, struct ac108_priv *ac108) {
	u8 i;
	for (i = 0; i < ac108->codec_index; i++) {
		printk("xx4.4xx ac108_update_bits   :0x%02x:0x%02x:%s\n",reg,val,ac108->i2c[i]->name);
...
static int ac108_configure_clocking(struct ac108_priv *ac108, unsigned int rate) {
	...
	if (ac108->clk_id == SYSCLK_SRC_PLL) {
		...
		printk("xx6xx ac108_configure_clocking  ac108_pll_div.n :0x%02x  PLL_LOOPDIV_LSB:0x%02x \n",(unsigned char)ac108_pll_div.n, PLL_LOOPDIV_LSB);
		...
		printk("xx6.1xx ac108_configure_clocking  ac108_pll_div.m1 :0x%02x  PLL_PREDIV1:0x%02x  ac108_pll_div.m2:0x%02x PLL_PREDIV2:0x%02x   \n", ac108_pll_div.m1 , PLL_PREDIV1, ac108_pll_div.m2,PLL_PREDIV2);
		 ...
		 * pll,enable sysclk 
		 */
		pr_err( "xx3xx ac108_set_sysclk ccc\n");
	...
	if (ac108->clk_id == SYSCLK_SRC_MCLK) {
		 ...
		 */
		pr_err("xx3xx ac108_set_sysclk bbb\n");
...
static int ac108_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params, struct snd_soc_dai *dai) {
	dev_info(dai->dev,"xx1xx ac108_hw_params\n");
	unsigned int i,j, channels, sample_resolution, rate;
	struct snd_soc_codec *codec = dai->codec;
	struct ac108_priv *ac108 = snd_soc_codec_get_drvdata(codec);
	rate = 99;
	dev_info(dai->dev, " xx1.1xx %s\n", __FUNCTION__);

	
	dev_info(dai->dev,"xx1.6xx ac108_hw_params:flags:%u \n", params->flags);
	dev_info(dai->dev,"xx1.6xx ac108_hw_params:rmask:%u \n", params->rmask);
	dev_info(dai->dev,"xx1.6xx ac108_hw_params:cmask:%u \n", params->cmask);
	dev_info(dai->dev,"xx1.6xx ac108_hw_params:info:%u \n", params->info);
	dev_info(dai->dev,"xx1.6xx ac108_hw_params:msbits:%u \n", params->msbits);
	dev_info(dai->dev,"xx1.6xx ac108_hw_params:rate_num:%u \n", params->rate_num);
	dev_info(dai->dev,"xx1.6xx ac108_hw_params:rate_den:%u \n", params->rate_den);
	dev_info(dai->dev,"xx1.6xx ac108_hw_params:reserved:==== \n");
	for(j=0;j<64;j++)
		{ 
		    dev_info(dai->dev,"%u", params->reserved[j]);
		}
	dev_info(dai->dev,"xx1.6xx ac108_hw_params:===== \n");
	...
	dev_info(dai->dev,"xx1.2xx \n");
	
	for (i = 0; i < ARRAY_SIZE(ac108_sample_rate); i++) {
		if (ac108_sample_rate[i].real_val ==( params_rate(params)/2)) {
			rate = i;
			dev_info(dai->dev,"xx1.2xx rate:%d \n", rate);
			break;
		}
	}
	if (rate == 99) return -EINVAL;

	dev_info(dai->dev, "xx1.3xx rate: %d , channels: %d , sample_resolution: %d",
	...
	dev_info(dai->dev,"xx1.44xx  ac108_sample_resolution[sample_resolution].reg_val:0x%02x  sample_resolution:0x%02x SLOT_WIDTH_SEL:0x%02x \n",ac108_sample_resolution[sample_resolution].reg_val,sample_resolution,SLOT_WIDTH_SEL);
	 ...
	 */
	dev_info(dai->dev,"xx1.5xx  ac108_sample_rate[rate].reg_val:%u ADC_FS_I2S1:u% val:%u \n",ac108_sample_rate[rate].reg_val,ADC_FS_I2S1,ac108_sample_rate[rate].reg_val << ADC_FS_I2S1); 
...
static int ac108_set_sysclk(struct snd_soc_dai *dai, int clk_id, unsigned int freq, int dir) {
	...
	case SYSCLK_SRC_MCLK:
		...
		break;
	case SYSCLK_SRC_PLL:
		dev_dbg(dai->dev, "xx3xx ac108_set_sysclk aaa   dai->name:%s \n",dai->name);
		pr_info("===ac108_set_sysclk 2===");
	...
	ac108->sysclk = freq;
	ac108->clk_id = clk_id;
	ac108->sysck_status=1;
	pr_info("===ac108_set_sysclk 3===");
	dev_dbg(dai->dev, "xx3xx ac108_set_sysclk\n");
	return 0;
}
...
static int ac108_set_fmt(struct snd_soc_dai *dai, unsigned int fmt) {
       ...
       dev_dbg(dai->dev, "xx2xx ac108_set_fmt \n");
	 
       dev_info(dai->dev, "xx2.1xx fmt: %u \n",fmt);
...
static int ac108_i2c_probe(struct i2c_client *i2c,
	...
	pr_info("xx0.1xx\n");
	pr_info(" i2c_id number :%u\n", (int)(i2c_id->driver_data));
	pr_info(" ac108  codec_index :%d\n", ac108->codec_index);
	pr_info(" ac108  I2S data protocol type :%u\n", ac108->data_protocol);
	...
	ac108_read(CHIP_RST, &read_val, i2c);
       pr_info("xx0.11xx r  0x%02x:0x%02x:0x%02x\n",CHIP_RST,read_val,CHIP_RST_VAL);
